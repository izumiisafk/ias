...
$stats = $conn->query("
    SELECT
        ...
        COUNT(DISTINCT CASE
            WHEN COALESCE(wh.total_hours, 0) > f.max_teaching_hours THEN f.faculty_id
        END) AS overloaded_faculty,
        COUNT(DISTINCT CASE
            WHEN COALESCE(wh.total_hours, 0) = 0 AND f.status = 'Active' THEN f.faculty_id
...
$all_scheds = $conn->query("
    SELECT s.faculty_id, s.day_of_week, s.start_time, s.end_time,
           sub.subject_name, sub.subject_code, sec.section_name
    FROM schedules s
...
<div class="main-content">
    ...
    <div class="content-card mb-4">
        <form method="GET" class="d-flex align-items-center gap-3 flex-wrap">
            ...
            <select name="department" class="form-select" style="max-width:340px;" onchange="this.form.submit()">
    ...
    <div class="content-card">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 style="color:var(--text-primary); font-weight:700; margin:0;">
                <i class="bi bi-bar-chart-fill me-2" style="color:var(--accent);"></i>Faculty Teaching Load
            </h5>
            ...
            <table class="custom-table">
                ...
                <tbody>
                    ...
                    <?php foreach ($faculty_rows as $row):
                        ...
                        if ($assigned == 0) {
                            $load_status = 'No Load';  $bar_color = '#94a3b8'; $badge_class = 'badge-secondary'; $load_class = 'load-none';
                        } elseif ($assigned > $max) {
                            $load_status = 'Overloaded'; $bar_color = '#ef4444'; $badge_class = 'badge-danger';  $load_class = 'load-over';
                        } elseif ($pct >= 80) {
                            $load_status = 'Near Limit'; $bar_color = '#f59e0b'; $badge_class = 'badge-warning'; $load_class = 'load-near';
                        } else {
                            $load_status = 'Normal';     $bar_color = '#22c55e'; $badge_class = 'badge-success'; $load_class = 'load-ok';
                        }
                    ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="facultyScheduleTitle"><i class="bi bi-calendar3 me-2"></i>Schedule</h5>
...
function viewFacultySchedule(facultyId, name) {
    ...
    if (scheds.length === 0) {
        ...
        scheds.forEach(s => {
            ...
            const dur   = ((parseInt(end.split(':')[0])*60+parseInt(end.split(':')[1]))
...
.badge-secondary { background:rgba(148,163,184,0.15); color:#94a3b8; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:600; }
